...
class SearchArticleBloc extends Bloc<SearchArticleEvent, SearchArticleState> {
  final SearchArticle _searchArticles;
      ...
      (event, emit) async {
        ...
        if (query.isEmpty) {
          ...
          result.fold((failure) => emit(SearchArticleError(failure.message)),
            ...
            emit(SearchArticleHasData(articlesData.articles ?? [],
                articlesData.totalResults ?? 0, 1));
        ...
        if (query.isEmpty) {
          ...
          result.fold((failure) => emit(SearchArticleError(failure.message)),
            ...
            emit(SearchArticleHasData(
                articles, articleData.totalResults ?? 0, page));
